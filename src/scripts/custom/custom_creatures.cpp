...
bool GossipHello_TeleportNPC(Player *player, Creature *_Creature)   
{
    ...
    if (player->GetTeam() == HORDE)
    {
        ...
        player->ADD_GOSSIP_ITEM(5, "小型副本"            , GOSSIP_SENDER_MAIN, 5);
        player->ADD_GOSSIP_ITEM(5, "团队副本"                 , GOSSIP_SENDER_MAIN, 101);
        player->ADD_GOSSIP_ITEM(5, "古拉巴什竞技场" , GOSSIP_SENDER_MAIN, 4015);
        player->ADD_GOSSIP_ITEM(5, "卡利姆"     , GOSSIP_SENDER_MAIN, 6010);
        player->ADD_GOSSIP_ITEM(5, "东部大陆"     , GOSSIP_SENDER_MAIN, 6020);
...
void SendDefaultMenu_TeleportNPC(Player *player, Creature *_Creature, uint32 action)
{
    switch (action)
    {
        case 1: // Cities [HORDE]
            ...
            player->ADD_GOSSIP_ITEM(5, "<-[主目录]"                  , GOSSIP_SENDER_MAIN, 100);
        ...
        case 5551: // Instances [PAGE 2]
            ...
            player->ADD_GOSSIP_ITEM(5, "沉没的寺庙" , GOSSIP_SENDER_MAIN, 1261);
        ...
        case 101: // Raids
            ...
            player->ADD_GOSSIP_ITEM(5, "奥妮克希亚巢穴" , GOSSIP_SENDER_MAIN, 4001);
        ...
        case 6010: // Kalimdor
            ...
            player->ADD_GOSSIP_ITEM(5, "寒冬之泉" , GOSSIP_SENDER_MAIN, 612);
            player->ADD_GOSSIP_ITEM(5, "<-[主目录]"                  , GOSSIP_SENDER_MAIN, 100);

            player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, _Creature->GetGUID());
            break;
        case 6020: // Eastern Kingdoms
            player->ADD_GOSSIP_ITEM(5, "阿拉希高低" , GOSSIP_SENDER_MAIN, 613);
            ...
            player->ADD_GOSSIP_ITEM(5, "银松树林" , GOSSIP_SENDER_MAIN, 621);
...
void Enchant(Player* player, Item* item, uint32 enchantid)
{
    if (!item)
    {
        player->GetSession()->SendNotification("你必须先装备你试图附魔的物品.");
        return;
    }

    if (!enchantid)
    {
        player->GetSession()->SendNotification("出了些问题.");
        return;
    }

    item->ClearEnchantment(PERM_ENCHANTMENT_SLOT);
    item->SetEnchantment(PERM_ENCHANTMENT_SLOT, enchantid, 0, 0);
    player->GetSession()->SendNotification("%s 成功附魔", item->GetProto()->Name1);
}
...
bool GossipSelect_EnchantNPC(Player* player, Creature* creature, uint32 sender, uint32 action)
{
    ...
    if (action < 20)
    {
        ...
        case EQUIPMENT_SLOT_CHEST:
            player->ADD_GOSSIP_ITEM(5, "强效属性", GOSSIP_SENDER_MAIN, CHEST_STATS);
            break;
        case EQUIPMENT_SLOT_BACK:
            ...
            player->ADD_GOSSIP_ITEM(5, "敏锐", GOSSIP_SENDER_MAIN, CLOAK_SUB);
        break;
        case EQUIPMENT_SLOT_WRISTS:
            player->ADD_GOSSIP_ITEM(5, "耐力", GOSSIP_SENDER_MAIN, BRACER_STAM);
            player->ADD_GOSSIP_ITEM(5, "力量", GOSSIP_SENDER_MAIN, BRACER_STR);
            player->ADD_GOSSIP_ITEM(5, "治疗", GOSSIP_SENDER_MAIN, BRACER_HEAL);
            player->ADD_GOSSIP_ITEM(5, "智力", GOSSIP_SENDER_MAIN, BRACER_INT);
            break;
        case EQUIPMENT_SLOT_HANDS:
            player->ADD_GOSSIP_ITEM(5, "敏捷", GOSSIP_SENDER_MAIN, GLOVES_AGI);
            player->ADD_GOSSIP_ITEM(5, "火焰能量", GOSSIP_SENDER_MAIN, GLOVES_FIRE);
            player->ADD_GOSSIP_ITEM(5, "寒冰能量", GOSSIP_SENDER_MAIN, GLOVES_FROST);
            player->ADD_GOSSIP_ITEM(5, "暗影能量", GOSSIP_SENDER_MAIN, GLOVES_SHADOW);
            player->ADD_GOSSIP_ITEM(5, "治疗", GOSSIP_SENDER_MAIN, GLOVES_HEALING);
            break;
        case EQUIPMENT_SLOT_FEET:
            player->ADD_GOSSIP_ITEM(5, "耐力", GOSSIP_SENDER_MAIN, BOOTS_STAM);
            player->ADD_GOSSIP_ITEM(5, "速度", GOSSIP_SENDER_MAIN, BOOTS_SPEED);
        ...
        case EQUIPMENT_SLOT_MAINHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "单手武器 敏捷", GOSSIP_SENDER_MAIN, WEP1H_AGILITY);
            player->ADD_GOSSIP_ITEM(5, "双手武器 敏捷", GOSSIP_SENDER_MAIN, WEP2H_AGILITY);
            player->ADD_GOSSIP_ITEM(5, "法术能量", GOSSIP_SENDER_MAIN, WEP_SPELLPOWER);
            player->ADD_GOSSIP_ITEM(5, "治疗", GOSSIP_SENDER_MAIN, WEP_HEAL);
            break;
        case EQUIPMENT_SLOT_OFFHAND:
            ...
            player->ADD_GOSSIP_ITEM(5, "抗性", GOSSIP_SENDER_MAIN, OFFHAND_FROSTRES);
            break;
        }
        player->SEND_GOSSIP_MENU(DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
    }
...
bool LearnAllRecipesInProfession(Player *pPlayer, SkillType skill)
{
    ...
    if (!SkillInfo)
    {
        sLog.outError("专业技能 NPC: received non-valid skill ID");
        return false;
    }

    pPlayer->SetSkill(SkillInfo->id, 300, 300);
    LearnSkillRecipesHelper(pPlayer, SkillInfo->id);
    pPlayer->GetSession()->SendNotification("学会了 %s 的所有配方.", skill_name);
    return true;
}
...
bool GossipHello_ProfessionNPC(Player* player, Creature* creature)
{
    ...
    player->ADD_GOSSIP_ITEM(GOSSIP_ICON_INTERACT_2, "炼金术",              GOSSIP_SENDER_MAIN, 1);
...
void CompleteLearnProfession(Player *pPlayer, Creature *pCreature, SkillType skill)
{
    ...
    {
        pPlayer->GetSession()->SendNotification("你已经学习了两个主要专业.");
    }
    else
    {
        if (!LearnAllRecipesInProfession(pPlayer, skill))
            pPlayer->GetSession()->SendNotification("内部错误.");
    }
}
